...
This writes `agent-action-plan.json` and `agent-action-plan.md`. The plan maps the latest `recommended_action` to a likely Sidecar evidence target and UI focus target, then repeats the manual execution boundary. In the web UI, `Focus Planned Action` opens the relevant drawer and focuses the suggested control, and the Agent Action Outcome fields are pre-filled as a pending manual-action draft. It does not execute the action, record the outcome automatically, or validate the agent recommendation.
...
The spotlight action `Use as observation anchor` only pre-fills the Visual Observation form from the dominant row. It sets a diagnostic observation type, `visual-diff.md`, role, diff view, visual anchor, and note, then focuses the note field. It does not call the record or guided-record endpoints; the operator must review the fields and explicitly record the observation.
