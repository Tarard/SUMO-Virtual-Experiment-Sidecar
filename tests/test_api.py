...
def test_homepage_uses_short_human_facing_session_urls(tmp_path: Path) -> None:
    app = create_app(adapter_factory=FakeAdapterFactory(), default_output_root=tmp_path / "runs")
    client = TestClient(app)
    ...
    assert script_response.status_code == 200
    assert "function sessionPageUrl" in script_response.text
    assert "/s/${encodeURIComponent(sessionId)}" in script_response.text
    assert "/s/${encodeURIComponent(state.sessionId)}/a/" in script_response.text
...
def test_homepage_exposes_agent_action_outcome_record(tmp_path: Path) -> None:
    app = create_app(adapter_factory=FakeAdapterFactory(), default_output_root=tmp_path / "runs")
    client = TestClient(app)
    ...
    assert index_response.status_code == 200
    assert script_response.status_code == 200
    assert "agentActionOutcomeAction" in index_response.text
    assert "agentActionOutcomeStatus" in index_response.text
    assert "recordAgentActionOutcomeBtn" in index_response.text
    assert "Record Agent Action Outcome" in index_response.text
    assert "agentActionOutcomePreview" in index_response.text
    assert "/agent-action-outcome/record" in script_response.text
    assert "agentActionOutcomePayload" in script_response.text
    assert "renderAgentActionOutcome" in script_response.text


def test_homepage_exposes_agent_loop_review_export(tmp_path: Path) -> None:
    app = create_app(adapter_factory=FakeAdapterFactory(), default_output_root=tmp_path / "runs")
    client = TestClient(app)
    ...
    assert index_response.status_code == 200
    assert script_response.status_code == 200
    assert "exportAgentLoopReviewBtn" in index_response.text
    assert "Export Agent Loop Review" in index_response.text
    assert "agentLoopReviewPreview" in index_response.text
    assert "/agent-loop-review/export" in script_response.text
...
def test_homepage_exposes_visual_diff_summary_strip(tmp_path: Path) -> None:
    app = create_app(adapter_factory=FakeAdapterFactory(), default_output_root=tmp_path / "runs")
    client = TestClient(app)
    ...
    script = script_response.text
    style = style_response.text
    assert "function renderVisualDiffSummary(visualDiff)" in script
    assert "function renderVisualDiffPairSummary(pair)" in script
    assert "function dominantVisualDiffRow(pair)" in script
    assert "visual-diff-summary" in script
    assert "visual-diff-summary-card" in script
    assert "dominant_row" in script
    assert "pixel_diff_status" in script
    ...
    assert render_function.index("renderVisualDiffSummary(visualDiff)") < render_function.index("renderVisualDiffMatrix(pair)")
    assert ".visual-diff-summary" in style
...
def test_short_session_routes_serve_page_and_artifacts(tmp_path: Path) -> None:
    baseline = tmp_path / "baseline.sumocfg"
    variant = tmp_path / "variant.sumocfg"
    baseline.write_text("<configuration/>", encoding="utf-8")
    variant.write_text("<configuration/>", encoding="utf-8")
    ...
    page_response = client.get(f"/s/{session_id}")
    artifact_response = client.get(f"/s/{session_id}/a/{relative_path}")
    blocked_response = client.get(f"/s/{session_id}/a/../baseline.sumocfg")

    assert page_response.status_code == 200
    assert "SUMO Virtual Experiment Sidecar" in page_response.text
    assert artifact_response.status_code == 200
    assert artifact_response.text
    assert blocked_response.status_code == 404
...
def test_agent_action_outcome_records_manual_result_from_action_plan(tmp_path: Path) -> None:
    ...
    create_response = client.post(
        "/api/session/create",
        json={
            "name": "agent-action-outcome-demo",
            "baseline_config": str(baseline),
            "variant_config": str(variant),
        },
    )
    ...
    assert response.status_code == 200
    body = response.json()
    outcome = body["agent_action_outcome"]
    assert outcome["label"] == "inspect-outputs-completed"
    assert outcome["action_plan_artifact"] == "agent-action-plan.md"
    assert outcome["action"] == "Inspect Outputs"
    assert outcome["outcome_status"] == "completed"
    assert outcome["evidence_artifact"] == "output-inspection.md"
    assert "completion evidence" in outcome["note"]
    assert Path(body["agent_action_outcome_json_path"]).name == "agent-action-outcomes.json"
    assert Path(body["agent_action_outcome_markdown_path"]).name == "agent-action-outcomes.md"
    assert "Output inspection was run" in body["agent_action_outcome_markdown"]
    assert "completed" in body["agent_action_outcome_markdown"]
    artifact_paths = {item["relative_path"] for item in body["evidence"]["artifacts"]}
    assert "agent-action-outcomes.json" in artifact_paths
    assert "agent-action-outcomes.md" in artifact_paths
    assert body["evidence"]["manifest"]["agent_action_outcomes"][0]["label"] == "inspect-outputs-completed"

    timeline_response = client.post(f"/api/session/{session_id}/timeline/export?preset=review")
    assert timeline_response.status_code == 200
    timeline = timeline_response.json()
    assert "agent-action-outcome" in {event["kind"] for event in timeline["timeline"]["events"]}
    assert "agent-action-outcomes.md" in timeline["timeline_markdown"]

    summary_response = client.post(f"/api/session/{session_id}/review/summary")
    assert summary_response.status_code == 200
    summary = summary_response.json()
    card_ids = {card["id"] for card in summary["review_summary"]["cards"]}
    assert "agent_action_outcomes" in card_ids
    assert "agent-action-outcomes.md" in summary["review_summary_markdown"]


def test_agent_loop_review_exports_closed_agent_control_loop(tmp_path: Path) -> None:
    baseline = tmp_path / "baseline.sumocfg"
    variant = tmp_path / "variant.sumocfg"
    baseline.write_text("<configuration/>", encoding="utf-8")
    variant.write_text("<configuration/>", encoding="utf-8")
    ...
    response = client.post(f"/api/session/{session_id}/agent-loop-review/export")

    assert response.status_code == 200
    body = response.json()
    review = body["agent_loop_review"]
    assert review["status"] == "outcome-recorded"
    step_statuses = {step["id"]: step["status"] for step in review["loop_steps"]}
    ...
    assert review["latest_feedback"]["label"] == "codex-output-check"
    assert review["latest_outcome"]["label"] == "inspect-outputs-completed"
    assert "Review recorded outcome evidence" in review["next_step"]
    assert Path(body["agent_loop_review_json_path"]).name == "agent-loop-review.json"
    assert Path(body["agent_loop_review_markdown_path"]).name == "agent-loop-review.md"
    assert "prompt -> feedback -> action plan -> outcome" in body["agent_loop_review_markdown"]
    assert "inspect-outputs-completed" in body["agent_loop_review_markdown"]
    assert "output-inspection.md" in body["agent_loop_review_markdown"]
    artifact_paths = {item["relative_path"] for item in body["evidence"]["artifacts"]}
    assert "agent-loop-review.json" in artifact_paths
    assert "agent-loop-review.md" in artifact_paths

    timeline_response = client.post(f"/api/session/{session_id}/timeline/export?preset=review")
    assert timeline_response.status_code == 200
    timeline = timeline_response.json()
    assert "agent-loop-review" in {event["kind"] for event in timeline["timeline"]["events"]}
    assert "agent-loop-review.md" in timeline["timeline_markdown"]
